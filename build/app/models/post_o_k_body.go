...
type PostOKBody struct {
	...
	// twitter
	Twitter []*PostOKBodyTwitterItems `json:"twitter"`
}
...
func (m *PostOKBody) validateTwitter(formats strfmt.Registry) error {
	if swag.IsZero(m.Twitter) { // not required
		return nil
	}

	for i := 0; i < len(m.Twitter); i++ {
		if swag.IsZero(m.Twitter[i]) { // not required
			continue
		}

		if m.Twitter[i] != nil {
			if err := m.Twitter[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("twitter" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("twitter" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
func (m *PostOKBody) contextValidateTwitter(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.Twitter); i++ {

		if m.Twitter[i] != nil {
			if err := m.Twitter[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("twitter" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("twitter" + "." + strconv.Itoa(i))
